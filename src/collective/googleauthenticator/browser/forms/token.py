...
class TokenForm(form.SchemaForm):
    ...
    ignoreContext = True
    schema = ITokenForm
    label = _(u'Codice di verifica autenticazione')
    description = _(u'''<p>Per creare il tuo primo account nell'app Google Authenticator associato al tuo utente del Portale sottoservizi, o ripristinarne uno già creato, puoi utilizzare il link <i>inviami il QR code</i>.<br>'''
        u''' Selezionando questo link ti verrà richiesto di digitare il tuo Nome utente del Portale sottoservizi e, alla mail collegata ad esso, ti arriverà un link per procedere alla creazione/ripristino '''
        u'''dell'account Google Authenticator.<br>Selezionando il link ricevuto per mail sul computer sarà visualizzato il QR code da scansionare con il cellulare nell'APP Google Authenticator.<br>'''
        u'''Scansionato il QR code sarà creato l'account «NomeUtente@sottoservizi.gruppoiren.it». Sotto il nome dell'account sarà visualizzato il codice di verifica da inserire sul computer '''
        u'''per completare la creazione/ripristino dell'account.</p>'''
        u'''<p>Nella sezione Notizie è disponibile un MANUALE OPERATIVO per l’installazione e l’uso dell’app Google Authenticator.</p>''')
    ...
    def updateFields(self, *args, **kwargs):
        ...
        if token_field:
            token_field.field.description = _(
                u'''<p>Il nome utente e la password digitate sono corrette. '''
                u'''Inserisci il codice di verifica (pin di 6 cifre) '''
                u'''generato dall'app Google Authenticator e CLICCA SU «Verifica» per completare l'accesso al Portale sottoservizi. <br>'''
                u'In caso di smarrimento o primo accesso puoi richiedere il nuovo codice al link '
                u'<a href=\"{0}/@@request-bar-code-reset\">inviami il QR code</a>.</p>'.format(
